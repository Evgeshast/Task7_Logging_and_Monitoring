...
namespace MvcMusicStore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    using (var counterHelper = PerformanceHelper.CreateCounterHelper<Counters>("Success"))
                    {
                        counterHelper.Increment(Counters.SuccessfulLogin);
                    }
                    return RedirectToLocal(returnUrl);
                }
                logger.Error($"Error during login using username:{model.UserName}, password:{model.Password}");
                using (var counterHelper = PerformanceHelper.CreateCounterHelper<Counters>("Error"))
                {
                    counterHelper.Increment(Counters.UnsuccessfulLogin);
                }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    logger.Info($"Registered {model.UserName}");
                    using (var counterHelper = PerformanceHelper.CreateCounterHelper<Counters>("Success"))
                    {
                        counterHelper.Increment(Counters.UserRegistration);
                    }
                    return RedirectToAction("Index", "Home");
                }
                logger.Error($"Not registered {model.UserName}");
                AddErrors(result);
            }
        ...
        {
            logger.Debug($"External {provider}");
